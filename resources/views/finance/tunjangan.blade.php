...
<style>
    ...
    /* Action Buttons */
    .btn-action {
        padding: 8px 16px; font-size: 0.85rem; font-weight: 600; border-radius: 6px;
    ...
    /* --- CONTAINER UNTUK TOMBOL AKSI (Supaya Tidak Bersatu) --- */
    .action-buttons {
        display: flex;
    ...
    table.modern-table td { padding: 15px; color: var(--text-grey); font-size: 0.95rem; vertical-align: middle; }
    table.modern-table td:first-child { font-weight: 600; color: var(--smart-maroon); }

    .currency { font-family: 'Roboto Mono', monospace; font-weight: 600; color: var(--text-dark); }
    ...
    <div class="card-box">
        ...
        <div class="welcome-card">
            ...
            <div class="welcome-text-content">
                ...
                <p>Kelola komponen tunjangan karyawan perusahaan.</p>
            </div>
        </div>
        ...
        <form action="{{ route('tunjangan.index') }}" method="GET" class="search-box">
            <input type="text"
                   name="search"
            ...
            @if(request('search'))
                <a href="{{ route('tunjangan.index') }}" style="text-decoration:none; color:#adb5bd; margin-left:10px; font-size:1.1rem;">
                ...
                <tbody>
                    ...
                    @foreach($tunjangan as $t)
                        <tr>
                            ...
                            <td><span class="currency">Rp {{ number_format($t->nominal, 0, ',', '.') }}</span></td>
                            <td>
                                <!-- ✅ DIBUNGKUS DIV AGAR SEJAJAR RAPI -->
                                <div class="action-buttons">
                                    <button class="btn-action btn-edit"
                                        onclick="openModal('edit', {{ $t->id_tunjangan }}, '{{ $t->nama_tunjangan }}', {{ $t->nominal }})">
                                        <i class="fas fa-pen"></i> Edit
                                    </button>

                                    <form action="{{ route('tunjangan.destroy',$t->id_tunjangan) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button class="btn-action btn-delete" onclick="return confirm('Yakin ingin menghapus tunjangan ini?')">
...
<!-- Modal -->
<div class="modal" id="tunjanganModal">
    <div class="modal-content">
        <div class="modal-header">
            <h3 id="modalTitle">Tambah Tunjangan</h3>
            <span style="cursor:pointer; color:white;" onclick="closeModal()"><i class="fas fa-times"></i></span>
        </div>

        <div class="modal-body">
            <form id="tunjanganForm" action="{{ route('tunjangan.store') }}" method="POST">
                @csrf

                <input type="hidden" name="id_tunjangan" id="tunjanganId">
        ...
        <div class="modal-footer">
            ...
            <button type="submit" form="tunjanganForm" class="btn btn-submit">Simpan</button>
        </div>
    </div>
</div>

<script>
    function openModal(mode, id = null, nama_tunjangan = '', nominal = '') {
        const modal = document.getElementById('tunjanganModal');
        const form = document.getElementById('tunjanganForm');
        ...
        if (mode === 'create') {
            form.action = "{{ route('tunjangan.store') }}";
            form.method = "POST";
            document.getElementById('methodInput').value = "POST";

            title.innerText = "Tambah Tunjangan";
            document.getElementById('tunjanganId').value = "";
            document.getElementById('nama_tunjangan').value = "";
            document.getElementById('nominal').value = "";

        } else {
            form.action = "{{ route('tunjangan.update', ':id') }}".replace(':id', id);
            form.method = "POST";
            document.getElementById('methodInput').value = "PUT";

            title.innerText = "Edit Tunjangan";
            document.getElementById('tunjanganId').value = id;
            document.getElementById('nama_tunjangan').value = nama_tunjangan;
